...
#SBATCH --job-name="faissR_VAMANA_CUDA"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_vamana_cuda_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_vamana_cuda_%j.err

set -euo pipefail


# CUDA faissR VAMANA tuning benchmark using precomputed exact references.
#
...
#   sbatch /scratch/firenze/NN/benchmark_scripts/run_hpc_vamana_tuning_cuda.sh
#
# The job scans float32 .RData files, uses explicit backend="cuda",
# method="vamana", Euclidean distance, k=15,30,50,100, and writes
...
export OUT_DIR="${OUT_DIR:-${BASE_DIR}/faissR_VAMANA_TUNING_CUDA_$(date +%Y%m%d_%H%M%S)}"
...

{
  ...
  echo "[$(date --iso-8601=seconds)] running CUDA VAMANA tuning from precomputed references"
  ...
  echo "DONE: ${OUT_DIR}"
} 2>&1 | tee -a "${OUT_DIR}/vamana_tuning_cuda.log"
