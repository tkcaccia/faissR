...
#SBATCH --job-name="faissR_SCANN_CPU12"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_scann_cpu12_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_scann_cpu12_%j.err

set -euo pipefail


# CPU-only faissR SCANN tuning benchmark using precomputed exact references.
#
...
#   sbatch /scratch/firenze/NN/benchmark_scripts/run_hpc_scann_tuning_cpu12.sh
#
# The job scans float32 .RData files, uses explicit backend="cpu",
# method="scann", Euclidean distance, k=10,15,50,100, and writes
...
export OUT_DIR="${OUT_DIR:-${BASE_DIR}/faissR_SCANN_TUNING_CPU12_$(date +%Y%m%d_%H%M%S)}"
...
BENCH_SCRIPT="$(resolve_script benchmark_scann_tuning_from_reference.R)"
...

{
  ...
  echo "[$(date --iso-8601=seconds)] running CPU-only SCANN tuning from precomputed references"
  ...
  echo "DONE: ${OUT_DIR}"
} 2>&1 | tee -a "${OUT_DIR}/scann_tuning_cpu12.log"
