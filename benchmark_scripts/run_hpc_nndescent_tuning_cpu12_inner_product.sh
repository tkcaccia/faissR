...
#SBATCH --job-name="faissR_NND_CPU12_IP"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_nndescent_cpu12_inner_product_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_nndescent_cpu12_inner_product_%j.err
...
if [[ -z "${OUT_DIR:-}" ]]; then
  export OUT_DIR="${BASE_DIR}/faissR_NNDESCENT_TUNING_CPU12_inner_product_$(date +%Y%m%d_%H%M%S)"
fi
