...
script_path <- function() {
  ...
  normalizePath(file.path("benchmark_scripts", "benchmark_flat_tuning_grid.R"), mustWork = FALSE)
}
...
cuda_candidates <- function(k, thread_values, output_values,
...
candidate_grid <- function(backend, k, thread_values, output_values,
  ...
  cuda_candidates(k, thread_values, output_values, gpu_batches, gpu_reuse_values)
}
...
run_rscript_task <- function(task, config, timeout, bench_script) {
  config_path <- tempfile("faissR_flat_config_", fileext = ".rds")
  result_path <- tempfile("faissR_flat_result_", fileext = ".rds")
...
summarize_results <- function(out_dir, target_recalls) {
  results_path <- file.path(out_dir, "flat_tuning_results.csv")
  ...
  write.csv(recommendations, file.path(out_dir, "flat_tuning_recommendations.csv"), row.names = FALSE)
  write.csv(provider_recommendations, file.path(out_dir, "flat_tuning_provider_recommendations.csv"), row.names = FALSE)
  ...
  write.csv(shape_candidates, file.path(out_dir, "flat_tuning_shape_candidates.csv"), row.names = FALSE)
  write.csv(shape_best, file.path(out_dir, "flat_tuning_shape_recommendations.csv"), row.names = FALSE)
...
write_report <- function(out_dir, results, recommendations, provider_recommendations) {
  lines <- c(
    ...
    "",
    "This benchmark evaluates public `method = \"flat\"` FAISS Flat exhaustive",
    "nearest-neighbour search for Euclidean self-KNN. Flat has no approximation",
    ...
    "## Tuned Parameters",
    "",
    "- CPU FAISS Flat: OpenMP/R thread count, FAISS query batch size, output distance storage, fitted-index cache mode.",
    "- CUDA FAISS GPU Flat: FAISS GPU query batch size, GPU resource reuse flag, output distance storage.",
    "- All rows record the resolved backend so unsupported FAISS/FAISS GPU builds are auditable.",
    "",
    "## Status Counts",
    "",
    ...
    "## Output Files",
    "",
    "- `flat_tuning_candidate_grid.csv`",
    "- `flat_tuning_results.csv`",
    "- `flat_tuning_recommendations.csv`",
    "- `flat_tuning_provider_recommendations.csv`",
    "- `flat_tuning_shape_candidates.csv`",
    "- `flat_tuning_shape_recommendations.csv`"
  )
  ...
  writeLines(lines, file.path(out_dir, "flat_tuning_report.md"))
}
...
main <- function() {
  ...
  out_dir <- args$out_dir %||% file.path(getwd(), paste0("faissR_FLAT_TUNING_", toupper(backend), "_", format(Sys.time(), "%Y%m%d_%H%M%S")))
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
  results_path <- file.path(out_dir, "flat_tuning_results.csv")
  candidate_path <- file.path(out_dir, "flat_tuning_candidate_grid.csv")
  ...
  write.csv(
    ...
    file.path(out_dir, "flat_tuning_config.csv"),
    row.names = FALSE
  )
  ...
  for (i in seq_len(nrow(manifest))) {
    ...
    if (!manifest_success(ds$status[[1L]]) || !file.exists(ds$path[[1L]])) {
      for (k in k_values) {
        candidates <- candidate_grid(backend, k, thread_values, output_values, cpu_batches, gpu_batches, cache_modes, gpu_reuse_values)
