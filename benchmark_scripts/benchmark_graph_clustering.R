...
`%||%` <- function(x, y) {
  ...
  if (!is.list(x) && length(x) == 1L && is.na(x)) return(y)
  x
}
...
default_graph_nn_methods <- function() {
  c("auto")
}
...
result_row <- function(dataset, n, p, cycle, k, graph_backend, cluster_backend, method,
                       ...
                       peak_rss_gb = NA_real_, graph_n_vertices = NA_integer_,
                       n_edges = NA_integer_,
                       ...
                       resolution_selected_is_min_gap = NA,
  ...
  data.frame(
    ...
    graph_n_vertices = as.integer(graph_n_vertices),
...
resolution_search_summary <- function(cluster) {
  selection <- cluster$resolution_selection %||% list()
  search <- if (is.null(cluster$resolution_search)) NULL else cluster$resolution_search
  selected_candidate <- selection$selected_candidate %||% NA_integer_
  n_candidates <- NA_integer_
  min_gap <- NA_integer_
  selected_is_min_gap <- NA
  if (is.data.frame(search) && nrow(search)) {
    n_candidates <- nrow(search)
    gaps <- suppressWarnings(as.integer(search$target_gap))
    finite_gaps <- gaps[!is.na(gaps)]
    if (length(finite_gaps)) {
      min_gap <- min(finite_gaps)
      selected_flag <- suppressWarnings(as.logical(search$selected))
      selected_rows <- which(!is.na(selected_flag) & selected_flag)
      if (length(selected_rows)) {
        selected_gap <- gaps[selected_rows[[1L]]]
        selected_is_min_gap <- !is.na(selected_gap) && identical(as.integer(selected_gap), as.integer(min_gap))
      }
    }
  }
  list(
...
compact_graph_metadata_value <- function(x) {
...
graph_route_parameters <- function(graph) {
  meta <- attr(graph, "faissR_graph") %||% list()
  sources <- list(
    nn_approximation = meta$nn_approximation %||% list(),
  ...
  keys <- c(
    ...
    "policy", "predicted_backend", "reason", "n", "p", "k", "work_size",
    "slow_tuning"
  )
  pieces <- character()
  for (source_name in names(sources)) {
    ...
    for (key in keys[keys %in% names(source)]) {
      value <- compact_graph_metadata_value(source[[key]])
      if (!is.na(value)) {
...
summarize_graph_cycles <- function(ok) {
  ...
  parts <- split(ok, paste(ok$dataset, ok$k, ok$graph_backend, ok$graph_method, ok$metric, ok$cluster_backend, ok$method, ok$weight, cluster_key, sep = "__"))
  summary <- lapply(parts, function(x) {
    data.frame(
      ...
      median_graph_n_vertices = finite_median(x$graph_n_vertices),
      ...
      resolution_selected_is_min_gap = all_true_or_na(x$resolution_selected_is_min_gap),
...
recommend_graph_cluster_methods <- function(cycle_summary, ari_tolerance) {
  ...
  parts <- split(
    ...
    paste(
      ...
      cycle_summary$metric, cycle_summary$cluster_backend,
      cluster_key,
      sep = "__"
    )
  )
...
build_graph_once <- function(data_obj, dataset_name, k, graph_backend, weight,
  ...
  tryCatch({
    ...
    list(
      ...
      graph = graph,
      graph_sec = elapsed,
      graph_n_vertices = graph$n_vertices %||% graph_meta$n_vertices %||% NA_integer_,
...
run_cluster_one <- function(data_obj, dataset_name, graph_obj, graph_sec,
  ...
  tryCatch({
    ...
    result_row(
      ...
      total_sec = total_sec,
      peak_rss_gb = read_peak_rss_gb(),
      graph_n_vertices = cluster_params$n_vertices %||% graph_n_vertices,
      n_edges = cluster_params$n_edges %||% n_edges,
...
graph_methods <- validate_choice_values(
  split_arg(args$graph_methods, paste(default_graph_nn_methods(), collapse = ",")),
  valid_graph_nn_methods(),
  "graph_methods"
)
...
config <- data.frame(
  key = c("data_root", "out_dir", "available_datasets", "datasets", "methods",
          "graph_methods", "metrics", "graph_backends", "cluster_backends",
              ...
              for (method in methods) {
                ...
                if (!is.null(skip_reason)) {
                  row <- result_row(
                    ...
                    peak_rss_gb = read_peak_rss_gb(),
                    graph_n_vertices = if (identical(graph_build$status, "success")) graph_build$graph_n_vertices else NA_integer_,
                  ...
                  row <- result_row(
                    ...
                    peak_rss_gb = read_peak_rss_gb(),
                    graph_n_vertices = graph_build$graph_n_vertices %||% NA_integer_,
                    graph_resolved_backend = if (identical(graph_build$status, "success")) graph_build$graph_resolved_backend else NA_character_,
                    graph_preflight_route = graph_build$graph_preflight_route %||% graph_preflight_route,
                  ...
                  row <- run_cluster_one(
                    ...
                    graph_sec = graph_build$graph_sec,
                    graph_cached = TRUE,
                    graph_n_vertices = graph_build$graph_n_vertices,
                ...
                cat(
                  sprintf(
                    "[%s] dataset=%s cycle=%s k=%s graph=%s/%s metric=%s cluster=%s method=%s status=%s total=%.3f\n",
                    format(Sys.time(), "%Y-%m-%d %H:%M:%S"),
                    dataset_name, cycle, k, graph_backend, graph_method, metric, cluster_backend, method,
...
if (nrow(ok)) {
  ...
  best_by_target <- select_graph_best_rows(ok, group_cols = c("dataset", "k", "graph_method", "metric", "n_clusters_requested"))
  utils::write.csv(
    best_by_target,
