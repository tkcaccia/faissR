...
#SBATCH --job-name="faissR_HNSW_CPU12_EUCL"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_hnsw_cpu12_euclidean_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_hnsw_cpu12_euclidean_%j.err
...
if [[ -z "${OUT_DIR:-}" ]]; then
  export OUT_DIR="${BASE_DIR}/faissR_HNSW_TUNING_CPU12_euclidean_$(date +%Y%m%d_%H%M%S)"
fi
