...
script_path <- function() {
  ...
  normalizePath(file.path("benchmark_scripts", "benchmark_ivf_tuning_grid.R"), mustWork = FALSE)
}
...
ivf_candidates <- function(n, k, backend, grid_level = "standard") {
  provider <- if (identical(backend, "cuda")) "faiss_gpu_ivf_flat" else "faiss_cpu_ivf_flat"
  auto <- data.frame(
  ...
  manual <- do.call(rbind, lapply(seq_len(nrow(specs)), function(i) {
    ...
    data.frame(
      candidate_id = sprintf(
        "%s_%s_nl%d_np%d",
        provider, specs$label[[i]], nlist, nprobe
      ),
  ...
  manual <- manual[!duplicated(paste(manual$requested_nlist, manual$requested_nprobe, sep = "\r")), , drop = FALSE]
  unique(rbind(auto, manual))
}

candidate_grid <- function(dataset_row, backend, k, grid_level) {
  ivf_candidates(
...
apply_candidate_options <- function(candidate) {
  opts <- list(
    ...
    faissR.faiss_gpu_ivf_tune = FALSE
  )
...
estimate_working_gb <- function(n, p, k, candidate, backend) {
  ...
  if (identical(backend, "cuda")) {
    return(2 * data_gb + centroid_gb + output_gb)
  }
  data_gb + centroid_gb + output_gb
}
...
run_rscript_task <- function(task, config, timeout, bench_script) {
  config_path <- tempfile("faissR_ivf_config_", fileext = ".rds")
  result_path <- tempfile("faissR_ivf_result_", fileext = ".rds")
...
summarize_results <- function(out_dir, target_recalls) {
  results_path <- file.path(out_dir, "ivf_tuning_results.csv")
  ...
  recommendations <- if (length(rec_rows)) {
  ...
  write.csv(recommendations, file.path(out_dir, "ivf_tuning_recommendations.csv"), row.names = FALSE)
  ...
  empty_shape_candidates <- data.frame(
    ...
    requested_nlist = integer(0),
    requested_nprobe = integer(0),
    stringsAsFactors = FALSE
  )
  ...
  write.csv(shape_candidates, file.path(out_dir, "ivf_tuning_shape_candidates.csv"), row.names = FALSE)
  write.csv(shape_best, file.path(out_dir, "ivf_tuning_shape_recommendations.csv"), row.names = FALSE)
...
write_report <- function(out_dir, results, recommendations) {
  lines <- c(
    ...
    "",
    "This benchmark evaluates explicit `method = \"ivf\"` settings for Euclidean self-KNN.",
    "CPU rows use FAISS CPU IndexIVFFlat. CUDA rows use FAISS GPU GpuIndexIVFFlat,",
    "which is the public `backend = \"cuda\", method = \"ivf\"` route in faissR.",
    ...
    "## Backend Notes",
    "",
    "- `backend = \"cpu\"`, `method = \"ivf\"` resolves to `faiss_ivf` and requires FAISS.",
    "- `backend = \"cuda\"`, `method = \"ivf\"` resolves to `faiss_gpu_ivf_flat` and requires FAISS GPU/cuVS support.",
    "- This tuning script disables FAISS GPU pilot tuning and measures only the explicit grid.",
    ...
    "## Output Files",
    "",
    "- `ivf_tuning_config.csv`",
    "- `ivf_tuning_candidate_grid.csv`",
    "- `ivf_tuning_results.csv`",
    "- `ivf_tuning_recommendations.csv`",
    "- `ivf_tuning_shape_candidates.csv`",
    "- `ivf_tuning_shape_recommendations.csv`"
  )
  ...
  writeLines(lines, file.path(out_dir, "ivf_tuning_report.md"))
}
...
main <- function() {
  ...
  out_dir <- args$out_dir %||% file.path(getwd(), paste0("faissR_IVF_TUNING_", toupper(backend), "_", format(Sys.time(), "%Y%m%d_%H%M%S")))
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
  results_path <- file.path(out_dir, "ivf_tuning_results.csv")
  candidate_path <- file.path(out_dir, "ivf_tuning_candidate_grid.csv")
  ...
  candidate_rows <- do.call(rbind, lapply(seq_len(nrow(manifest)), function(i) {
    ...
    rows <- do.call(rbind, lapply(k_values, function(k) {
      x <- candidate_grid(ds, backend, k, grid_level)
  ...
  write.csv(
    ...
    file.path(out_dir, "ivf_tuning_config.csv"),
    row.names = FALSE
  )
  ...
  for (i in seq_len(nrow(manifest))) {
    ...
    if (!manifest_success(ds$status[[1L]]) || !file.exists(ds$path[[1L]])) {
      for (k in k_values) {
        write_missing_rows(
          ...
          backend,
          k,
          candidate_grid(ds, backend, k, grid_level),
    ...
    for (k in k_values) {
      candidates <- candidate_grid(ds, backend, k, grid_level)
