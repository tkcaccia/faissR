...
#SBATCH --job-name="faissR_SCANN_CUDA"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_scann_cuda_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_scann_cuda_%j.err

set -euo pipefail


# CUDA faissR SCANN tuning benchmark using precomputed exact references.
#
...
#   sbatch /scratch/firenze/NN/benchmark_scripts/run_hpc_scann_tuning_cuda.sh
#
...
export OUT_DIR="${OUT_DIR:-${BASE_DIR}/faissR_SCANN_TUNING_CUDA_$(date +%Y%m%d_%H%M%S)}"
...

{
  ...
  echo "[$(date --iso-8601=seconds)] running CUDA SCANN tuning from precomputed references"
  ...
  echo "DONE: ${OUT_DIR}"
} 2>&1 | tee -a "${OUT_DIR}/scann_tuning_cuda.log"
