...
#SBATCH --job-name="faissR_B1_CPU_EUCL"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_benchmark1_compare_cpu12_euclidean_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_benchmark1_compare_cpu12_euclidean_%j.err

set -euo pipefail

# Benchmark #1 CPU-only Euclidean comparison.
#
# This job compares faissR CPU methods with CPU external R KNN packages only.
# It excludes CUDA methods and graph/consumer rows so the speed table compares
# KNN-search outputs on the same CPU resources.
...
export OUT_DIR="${OUT_DIR:-${BASE_DIR}/faissR_BENCHMARK1_COMPARE_CPU12_EUCLIDEAN_$(date +%Y%m%d_%H%M%S)}"
...

{
  ...
  echo "[$(date --iso-8601=seconds)] running Benchmark #1 CPU Euclidean comparison"
  ...
  echo "DONE: ${OUT_DIR}"
} 2>&1 | tee -a "${OUT_DIR}/benchmark1_compare_cpu12_euclidean.log"
