...
script_path <- function() {
  ...
  normalizePath(file.path("benchmark_scripts", "benchmark_ivfpq_tuning_grid.R"), mustWork = FALSE)
}
...
gpu_supported_pq_m <- function() {
  c(1L, 2L, 3L, 4L, 8L, 12L, 16L, 20L, 24L, 28L, 32L, 48L)
}

divisor_at_most <- function(p, target, allowed = NULL) {
  p <- as.integer(max(1L, p))
  target <- as.integer(max(1L, min(p, target)))
  vals <- seq_len(target)
  divs <- vals[p %% vals == 0L]
  if (!is.null(allowed)) divs <- intersect(divs, allowed)
  if (length(divs)) max(divs) else 1L
}

pq_m_for_mode <- function(p, mode, backend) {
  target <- switch(
    mode,
    speed = 8L,
    balanced = 16L,
  ...
  if (identical(backend, "cuda")) {
    return(divisor_at_most(p, min(target, 48L), gpu_supported_pq_m()))
  }
  divisor_at_most(p, target)
}

parse_pq_bits_values <- function(value, backend, grid_level) {
  if (identical(backend, "cuda")) return(8L)
  default <- switch(
    grid_level,
    compact = "4,8",
    wide = "4,6,8",
  ...
  vals <- suppressWarnings(as.integer(split_arg(value, default)))
  vals <- unique(vals[is.finite(vals) & vals >= 4L & vals <= 12L])
  if (!length(vals)) vals <- c(4L, 8L)
  vals
}

ivfpq_candidates <- function(n, p, k, backend, grid_level = "standard",
                             pq_bits_values = 8L) {
  provider <- if (identical(backend, "cuda")) "faiss_gpu_ivfpq" else "faiss_cpu_ivfpq"
  auto <- data.frame(
    ...
    requested_pq_nbits = NA_integer_,
    ...
    for (nbits in pq_bits_values) {
      ...
      manual_rows[[idx]] <- data.frame(
        candidate_id = sprintf(
          "%s_%s_nl%d_np%d_m%d_b%d",
          provider, specs$label[[i]], nlist, nprobe, pq_m, nbits
        ),
        ...
        requested_pq_m = pq_m,
        requested_pq_nbits = as.integer(nbits),
        stringsAsFactors = FALSE
      )
    }
  }
  ...
  dedupe_key <- paste(
    manual$requested_nlist, manual$requested_nprobe,
    manual$requested_pq_m, manual$requested_pq_nbits,
    sep = "\r"
  )
...
candidate_grid <- function(dataset_row, backend, k, grid_level, pq_bits_values) {
  ivfpq_candidates(
...
apply_candidate_options <- function(candidate) {
  ...
  if (!identical(candidate$candidate_kind, "auto")) {
    ...
    opts$faissR.faiss_pq_m <- as.integer(candidate$requested_pq_m)
    opts$faissR.faiss_pq_nbits <- as.integer(candidate$requested_pq_nbits)
  }
...
estimate_working_gb <- function(n, p, k, candidate, backend) {
  ...
  pq_m <- as.numeric(candidate$requested_pq_m)
  nbits <- as.numeric(candidate$requested_pq_nbits)
  data_gb <- n * p * 4 / 1024^3
  centroid_gb <- nlist * p * 4 / 1024^3
  code_gb <- n * pq_m * nbits / 8 / 1024^3
  output_gb <- n * (k + 1) * 12 / 1024^3
  if (identical(backend, "cuda")) {
    return(2 * data_gb + centroid_gb + code_gb + output_gb)
  }
  data_gb + centroid_gb + code_gb + output_gb
}
...
base_row <- function(config, status = "success", error = NA_character_) {
  ...
  data.frame(
    ...
    requested_pq_nbits = as.integer(candidate$requested_pq_nbits),
    ...
    tuning_metric = NA_character_,
    tuning_metric_aware = NA,
    tuning_source = NA_character_,
    pq_tuning_policy = NA_character_,
    pq_tuning_rule = NA_character_,
    pq_tuning_source = NA_character_,
...
run_method <- function(config) {
  ...
  row$role <- approx$role %||% NA_character_
  row$tuning_policy <- approx$tuning_policy %||% NA_character_
  row$tuning_rule <- approx$tuning_rule %||% NA_character_
  row$tuning_metric <- approx$tuning_metric %||% NA_character_
  row$tuning_metric_aware <- isTRUE(approx$tuning_metric_aware)
  row$tuning_source <- approx$tuning_source %||% NA_character_
  row$pq_tuning_policy <- approx$pq_tuning_policy %||% NA_character_
  row$pq_tuning_rule <- approx$pq_tuning_rule %||% NA_character_
  row$pq_tuning_source <- approx$pq_tuning_source %||% NA_character_
...
run_rscript_task <- function(task, config, timeout, bench_script) {
  config_path <- tempfile("faissR_ivfpq_config_", fileext = ".rds")
  result_path <- tempfile("faissR_ivfpq_result_", fileext = ".rds")
...
summarize_results <- function(out_dir, target_recalls) {
  results_path <- file.path(out_dir, "ivfpq_tuning_results.csv")
  ...
  write.csv(recommendations, file.path(out_dir, "ivfpq_tuning_recommendations.csv"), row.names = FALSE)
  ...
  if (nrow(success)) {
    ...
    for (target in target_recalls) {
      ...
      for (part in split(success, split_key)) {
        ...
        shape_rows[[idx]] <- data.frame(
          ...
          requested_pq_nbits = as.integer(part$requested_pq_nbits[[1L]]),
          stringsAsFactors = FALSE
        )
      }
    }
    ...
    write.csv(shape_candidates, file.path(out_dir, "ivfpq_tuning_shape_candidates.csv"), row.names = FALSE)
    ...
    write.csv(shape_best, file.path(out_dir, "ivfpq_tuning_shape_recommendations.csv"), row.names = FALSE)
  }
...
write_report <- function(out_dir, results, recommendations) {
  lines <- c(
    ...
    "",
    "This benchmark evaluates explicit `method = \"ivfpq\"` settings for Euclidean self-KNN.",
    "CPU rows use FAISS CPU IndexIVFPQ. CUDA rows use FAISS GPU GpuIndexIVFPQ,",
    "which is the public `backend = \"cuda\", method = \"ivfpq\"` route in faissR.",
    "Recommendation tables select the fastest successful candidate reaching recall",
    "thresholds 0.90, 0.95, and 0.99, or the highest-recall candidate when no row",
    ...
    "- `faiss_pq_m`: product-quantizer subquantizer count; FAISS may adjust it to divide the feature dimension.",
    "- `faiss_pq_nbits`: CPU codebook bits. FAISS GPU IVFPQ forces 8-bit PQ codes in the current C++ backend.",
    "",
    "## Backend Notes",
    "",
    "- `backend = \"cpu\"`, `method = \"ivfpq\"` resolves to `faiss_ivfpq` and requires FAISS.",
    "- `backend = \"cuda\"`, `method = \"ivfpq\"` resolves to `faiss_gpu_ivfpq` and requires FAISS GPU/cuVS support.",
    "- CPU IVFPQ needs at least 624 training rows. GPU IVFPQ needs at least 256 rows.",
    "- This tuning script uses Euclidean/L2 only.",
    "",
    "## Status Counts",
    "",
    ...
    "## Output Files",
    "",
    "- `ivfpq_tuning_config.csv`",
    "- `ivfpq_tuning_candidate_grid.csv`",
    "- `ivfpq_tuning_results.csv`",
    "- `ivfpq_tuning_recommendations.csv`",
    "- `ivfpq_tuning_shape_candidates.csv`",
    "- `ivfpq_tuning_shape_recommendations.csv`"
  )
  ...
  writeLines(lines, file.path(out_dir, "ivfpq_tuning_report.md"))
}
...
main <- function() {
  ...
  out_dir <- args$out_dir %||% file.path(getwd(), paste0("faissR_IVFPQ_TUNING_", toupper(backend), "_", format(Sys.time(), "%Y%m%d_%H%M%S")))
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
  results_path <- file.path(out_dir, "ivfpq_tuning_results.csv")
  candidate_path <- file.path(out_dir, "ivfpq_tuning_candidate_grid.csv")
  ...
  pq_bits_values <- parse_pq_bits_values(args$pq_bits_values, backend, grid_level)
  ...
  candidate_rows <- do.call(rbind, lapply(seq_len(nrow(manifest)), function(i) {
    ...
    rows <- do.call(rbind, lapply(k_values, function(k) {
      x <- candidate_grid(ds, backend, k, grid_level, pq_bits_values)
  ...
  write.csv(
    data.frame(
      key = c("backend", "manifest", "datasets", "k_values", "target_recalls",
              ...
              "pq_bits_values", "reference_backend", "max_working_gb"),
      value = c(backend, manifest_path, paste(datasets, collapse = ","),
                ...
                paste(pq_bits_values, collapse = ","), reference_backend, max_working_gb)
    ),
    file.path(out_dir, "ivfpq_tuning_config.csv"),
    row.names = FALSE
  )
  ...
  for (i in seq_len(nrow(manifest))) {
    ...
    if (!manifest_success(ds$status[[1L]]) || !file.exists(ds$path[[1L]])) {
      for (k in k_values) {
        write_missing_rows(
          ...
          backend,
          k,
          candidate_grid(ds, backend, k, grid_level, pq_bits_values),
    ...
    for (k in k_values) {
      candidates <- candidate_grid(ds, backend, k, grid_level, pq_bits_values)
