...
| `"cagra"` | Unsupported. | FAISS GPU CAGRA preferred; direct RAPIDS cuVS CAGRA when available. `options(faissR.cagra_implementation = "faiss_gpu")` or `"cuvs"` forces one provider; `"auto"` keeps the default FAISS-then-cuVS provider rule. Direct cuVS CAGRA additionally exposes `cagra_build_algo` / `options(faissR.cuvs_cagra_build_algo = ...)` for the cuVS graph builder (`"auto"`, `"ivf_pq"`, `"nn_descent"`, or `"iterative_cagra_search"`); `cagra_build_algo = "auto"` is faissR's shape-aware rule and uses iterative CAGRA construction for compact high-dimensional self-KNN cases. Availability checks respect the forced provider for every metric. Approximation metadata records `cagra_provider` (`"faiss_gpu"` or `"cuvs"`), `cagra_provider_option`, and direct-cuVS `cagra_build_algo`. Cosine/correlation use normalized Euclidean graph search. Raw inner-product CAGRA is available only through FAISS GPU CAGRA; direct cuVS CAGRA inner product is disabled until the transformed route is safe. | CUDA graph-search method via FAISS/cuVS CAGRA [3,13-16]. |
