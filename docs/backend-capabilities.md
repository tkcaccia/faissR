...
| RAPIDS cuVS HNSW | no | yes, if cuVS is built with HNSW headers | CUDA-built CAGRA index converted to a base-layer cuVS HNSW index (`hnsw_hierarchy = "none"`) and searched through the cuVS HNSW wrapper; supports Euclidean/L2, normalized cosine/correlation, and transformed raw inner product in faissR. Automatic cuVS HNSW uses `iterative_cagra_search` for the internal CAGRA seed graph because the HNSW conversion needs high seed-graph recall, then uses a recall-0.99 search-effort default (`ef = max(50, k)`) for speed; explicit `cagra_build_algo = "ivf_pq"` remains available for experiments and records the selected builder in approximation metadata [3,5,22-23]. |
