...
| `"nndescent"` | Native CPU NNDescent for Euclidean/L2, cosine, correlation, and raw inner product. | Direct RAPIDS cuVS NN-descent for Euclidean/L2, cosine, and correlation; faissR native CUDA candidate refinement for raw inner product. | Approximate KNN graph construction; cosine/correlation use normalized Euclidean search, raw inner product uses shifted dot-product distances, native CPU candidate expansion stores reverse neighbours in flat fixed-width C++ buffers, and FAISS NNDescent is disabled by default because linked FAISS builds can abort during graph construction. CUDA Euclidean auto tuning uses measured cuVS rows; CUDA cosine and correlation currently use seeded policies derived from those rows and report validation-pending metadata, while CUDA raw inner product uses faissR's native shifted-dot-product candidate refinement route because direct cuVS NN-descent does not expose raw IP [3-4,16]. |
