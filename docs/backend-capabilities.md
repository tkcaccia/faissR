...
| `"exact"` | FAISS Flat L2 for Euclidean, normalized FAISS Flat cosine for cosine, centered/normalized FAISS Flat correlation for correlation, and FAISS Flat IP for raw inner product when FAISS is available; native exact CPU fallback. | FAISS GPU Flat if available; otherwise direct cuVS brute force for exact metric-aware search when cuVS is available. | Exact/high-recall baseline with CPU Euclidean/cosine/correlation/inner-product compiled shape/k/target tuning metadata and CUDA Euclidean/cosine/correlation exact query-batch/resource metadata from the metric-specific CUDA sweeps, including `benchmark_scripts/cuda_exact_correlation_shape_tuning_defaults_from_uploaded_results.csv` [1-3,16]. |
| `"flat"` | FAISS Flat L2/IP; cosine and correlation use normalized Flat IP, with exact CPU fallback for zero-normalized rows. | FAISS GPU Flat L2/IP; cosine and correlation use normalized Flat IP; degenerate zero-normalized rows error instead of being repaired on CPU. | Exact FAISS exhaustive search with CPU Euclidean/cosine/correlation/inner-product compiled shape/k/target tuning metadata and CUDA Euclidean/cosine/correlation Flat query-batch/resource metadata from the metric-specific sweeps, including `benchmark_scripts/cuda_flat_correlation_shape_tuning_defaults_from_uploaded_results.csv` [1-2,16]. |
| `"bruteforce"` | FAISS Flat L2 for Euclidean, normalized FAISS Flat cosine for cosine, centered/normalized FAISS Flat correlation for correlation, and FAISS Flat IP for raw inner product when FAISS is available; native exact CPU fallback. | Direct RAPIDS cuVS brute force when available; cosine/correlation use normalized Euclidean search and inner product uses a maximum-inner-product-to-L2 transform around the cuVS L2 kernel. | Exhaustive route with CPU Euclidean/cosine/correlation/inner-product compiled tuning metadata and CUDA Euclidean/cosine/correlation cuVS query-batch/resource metadata, useful for FAISS/cuVS comparisons [1-3,16]. |
