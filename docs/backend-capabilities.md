...
route when native CUDA kernels are available; explicit CUDA CAGRA can use the
MIPS-to-L2 graph-search transform, while CAGRA-derived cuVS HNSW is not used
for raw inner product. On a cuVS-only runtime, CUDA auto
