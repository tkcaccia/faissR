...
| `"ivfpq_fastscan"` | FAISS CPU `IndexIVFPQFastScan` with 4-bit PQ and optional Flat refinement. CPU supports Euclidean/L2, raw inner product through FastScan IP, cosine through row L2 normalization, and correlation through row centering plus L2 normalization followed by FastScan L2 search. | Direct RAPIDS cuVS IVF-PQ with 4-bit compressed codes and fitted-index/resource/query-buffer reuse. CUDA FastScan is Euclidean/L2-only. | IVFPQ FastScan compressed-code scan. CPU requires linked FAISS FastScan headers; CUDA requires cuVS and does not silently fall back to CPU [6,34]. |
...
| RAPIDS cuVS 4-bit IVF-PQ | no | yes, if cuVS is built | Public CUDA `method = "ivfpq_fastscan"` route. Uses direct cuVS IVF-PQ with 4-bit compressed codes for Euclidean search, keeps compatible fitted indexes, dataset device buffers, query device buffers, and cuVS resources in bounded session caches where possible, and records `cuda_cuvs_ivfpq_fastscan` metadata; it does not use CPU FastScan as a fallback [3,6,34]. |
