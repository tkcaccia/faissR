...
| RAPIDS cuVS HNSW | no | yes, if cuVS is built with HNSW headers | CUDA-built CAGRA index converted to a base-layer cuVS HNSW index (`hnsw_hierarchy = "none"`) and searched through the cuVS HNSW wrapper; supports Euclidean/L2, normalized cosine/correlation, and transformed raw inner product in faissR. Automatic cuVS HNSW uses `iterative_cagra_search` for the internal CAGRA seed graph because the HNSW conversion needs high seed-graph recall, then selects graph/search effort from the requested `target_recall` tier (`0.9`, `0.95`, or `0.99`); explicit graph-degree, intermediate-degree, `ef`, and `cagra_build_algo = "ivf_pq"` options remain available for experiments and record the selected values in approximation metadata [3,5,22-23]. |
