...
| `"nsg"` | Navigating Spreading-out Graph style approximate search. CPU uses FAISS NSG for Euclidean/L2 when exposed by the linked FAISS build, and faissR's native NSG-style self-KNN route for cosine, correlation, and inner product. CUDA uses faissR's native NSG-style self-KNN route for all public metrics, with normalized cosine/correlation and shifted dot-product inner-product distances. Native NSG `tuning = "auto"` uses backend-specific shape/k/metric defaults and reads `faissR.cpu_nsg_*` or `faissR.cuda_nsg_*` options [16,21,29]. |
...
| `"cagra"` | CUDA-only graph-search method. faissR prefers FAISS GPU CAGRA when FAISS is built with NVIDIA cuVS integration and otherwise uses direct cuVS CAGRA when available. Set `options(faissR.cagra_implementation = "faiss_gpu")` or `"cuvs"` to force the provider for benchmarking; `"auto"` keeps the default FAISS-then-cuVS rule. It supports Euclidean/L2 plus cosine/correlation through normalized Euclidean graph search; raw inner product is not exposed [3,13-16]. |
