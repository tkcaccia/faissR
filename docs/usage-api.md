...
exact search has recall 1 by construction. With `method = "auto"`, the object
also records `auto_preferred_backend`, `auto_preferred_method`, and
`auto_residency_constraint` when ordinary `nn()` would choose an approximate
CUDA backend that is not yet exposed as a persistent GPU-resident result.
