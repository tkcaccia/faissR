...
| `metric` | Distance metric passed to `nn(..., exclude_self = TRUE)` when faissR needs to build the KNN graph internally; aliases such as `"l2"`, `"cor"`/`"pearson"`, and `"ip"` are accepted. Correlation is centered cosine similarity, not raw inner product. Inner-product graph construction ranks by larger raw dot product and reuses faissR's shifted smaller-is-better distance convention from `nn()`. |
...
| `tuning` | Tuning policy passed to `nn()`. `"auto"` uses the deterministic default for the resolved method; `"cache"` and `"pilot"` opt into pilot tuning where implemented. CPU Euclidean/cosine/correlation/inner-product IVF, IVFPQ, NN-descent, NSG, and Vamana use compiled shape/k/target tiers where available; IVFPQ FastScan has its own Euclidean table, while FastScan cosine, correlation, and inner product are marked validation-pending until the corrected metric-specific sweeps are rerun. FAISS GPU IVF pilot/cache tuning is Euclidean-only. |
