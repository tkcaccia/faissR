...
| `method` | GPU-resident method selector. Currently `"auto"`, `"exact"`, `"flat"`, and `"bruteforce"` all use exact GPU search. |
...
`"exact"`, `"flat"`, or `"bruteforce"`. Euclidean uses FAISS GPU direct
`bfKnn`; cosine, correlation, and raw inner-product use the native CUDA
GPU-resident exact route. `target_recall` is recorded for API symmetry but
exact search has recall 1 by construction. Approximate FAISS GPU/cuVS methods
still return host objects through `nn()` until their provider result buffers
...
| `target_recall` | Speed/recall tier passed to `nn(..., exclude_self = TRUE)` when faissR computes neighbours internally; affects CPU Euclidean/cosine/correlation/inner-product IVF and IVFPQ probing/PQ tiers, CUDA auto Flat-vs-IVF selection, CUDA IVF probing, CUDA Euclidean/correlation IVFPQ probing/PQ tiers, and HNSW tiers. |
...
| `tuning` | Tuning policy passed to `nn()`. `"auto"` uses the deterministic default for the resolved method; `"cache"` and `"pilot"` opt into pilot tuning where implemented. CPU Euclidean/cosine/correlation/inner-product IVF, IVFPQ, NN-descent, NSG, and Vamana use compiled shape/k/target tiers where available; CUDA IVF has compiled Euclidean, cosine, and correlation tiers; CUDA IVFPQ has compiled Euclidean and correlation tiers; CUDA CAGRA has a measured Euclidean table plus validation-pending cosine/correlation policies seeded from that table; CUDA NSG and Vamana have compiled Euclidean/cosine tiers plus validation-pending correlation policies seeded from the corresponding cosine tables; CUDA NN-descent has a compiled Euclidean table plus cosine and correlation policies seeded from that table until the corrected metric-specific sweeps are rerun; CUDA IVFPQ FastScan has a compiled Euclidean table plus cosine and correlation policies seeded from that table until the corrected metric-specific sweeps are rerun; FastScan raw inner product remains unsupported for CUDA. FAISS GPU IVF pilot/cache tuning is Euclidean-only. |
| `target_recall` | Speed/recall tier passed to `nn()` for fitting and immediate prediction; affects CPU Euclidean/cosine/correlation/inner-product IVF, IVFPQ, NSG, and Vamana, CUDA auto Flat-vs-IVF selection, CUDA IVF probing, CUDA Euclidean/correlation IVFPQ probing/PQ tiers, CUDA NSG/Vamana graph tiers, and HNSW tiers. |
...
| `target_recall` | Optional speed/recall tier for prediction; `NULL` reuses the fitted model setting. It affects CPU Euclidean/cosine/correlation/inner-product IVF and IVFPQ probing/PQ tiers, CUDA auto Flat-vs-IVF selection, CUDA IVF probing, CUDA Euclidean/correlation IVFPQ probing/PQ tiers, and HNSW tiers when a new NN search is needed. |
