...
| `"nsg"` | Navigating Spreading-out Graph style approximate search. CPU uses FAISS NSG for Euclidean/L2 when exposed by the linked FAISS build, and faissR's native NSG-style self-KNN route for cosine, correlation, and inner product. CUDA uses faissR's native NSG-style self-KNN route for all public metrics, with normalized cosine/correlation and shifted dot-product inner-product distances [16,21,29]. |
