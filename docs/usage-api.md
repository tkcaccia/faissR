...
| `metric` | Distance metric: `"euclidean"`, `"cosine"`, `"correlation"`, or `"inner_product"`. Euclidean/L2 is the validated high-performance route for approximate FAISS/CUDA/cuVS. Cosine and correlation use validated exact paths, FAISS CPU/GPU Flat, IVF-Flat, IVFPQ, and CPU HNSW through normalized inner-product search; NSG and CAGRA use normalized Euclidean graph search. Inner product is supported by native exact CPU scoring, FAISS Flat IP, FAISS IVF-Flat/IVFPQ IP, FAISS HNSW IP, and RcppHNSW/hnswlib fallback paths when FAISS is unavailable. NNDescent remains Euclidean-only because linked FAISS builds can abort during non-Euclidean graph construction. |
...
| `"nsg"` | FAISS CPU NSG graph-search index when the linked FAISS build exposes it. It supports Euclidean/L2 plus cosine/correlation through normalized Euclidean graph search; raw inner product is not exposed [16]. |
| `"nndescent"` | NN-descent style approximate graph construction. CPU uses faissR's native NNDescent route by default; CUDA maps to cuVS NN-descent. FAISS NNDescent is disabled by default because linked FAISS builds can abort during graph construction. Public NNDescent routes are kept Euclidean/L2-only [3-4,16]. |
