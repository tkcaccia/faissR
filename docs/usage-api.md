...
| `metric` | Distance metric: `"euclidean"`, `"cosine"`, `"correlation"`, or `"inner_product"`. Euclidean/L2 is the validated high-performance route for approximate FAISS/CUDA/cuVS. Cosine and correlation use validated exact paths, FAISS CPU/GPU Flat and IVF-Flat through normalized inner-product search, and CPU FAISS HNSW through normalized inner-product search when `method = "hnsw"` is selected. Inner product is supported by native exact CPU scoring, FAISS Flat IP, FAISS IVF-Flat IP, FAISS HNSW IP, and RcppHNSW/hnswlib fallback paths when FAISS is unavailable. |
