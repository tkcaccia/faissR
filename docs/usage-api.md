...
| --- | --- |
| `data` | Numeric matrix, data frame, or optional `float::fl()`/`float32` matrix with reference observations in rows and features in columns. CPU FAISS Flat, CPU FAISS HNSW, and CUDA cuVS HNSW requests use direct float-pointer input routes without converting the float32 source object to an R double matrix. Other resolved routes accept float32 datasets through a one-time compatibility conversion to R numeric and record `float32_compatibility_conversion = TRUE`. |
...
| `output` | Distance storage type: `"double"` returns the default R numeric matrix; `"float"` returns `distances` as a `float::fl()`/`float32` matrix and records `distance_type = "float32"` plus `attr(result, "distance_type") = "float32"`. Float32 inputs can use direct float-pointer input routes for CPU FAISS Flat, CPU FAISS HNSW, and CUDA cuVS HNSW; ordinary R double inputs on CPU FAISS Flat/HNSW float-output requests use a float-pointer FAISS route. Other routes that receive float32 input run through compatibility conversion and can still return float distances when `output = "float"`. Float32-route results expose `input_layout`, `input_owns_data`, and, when applicable, `float32_compatibility_conversion`. The `float` package is optional and used only when this output is requested or a float32 input object is supplied. |
