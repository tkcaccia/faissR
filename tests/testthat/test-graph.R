...
test_that("knn_graph passes method metric and tuning to internal KNN", {
  ...
  expected_backend <- if (faiss_available()) "faiss_flat_correlation" else "cpu"
  expect_equal(meta$nn_backend, expected_backend)
  expect_equal(meta$requested_backend, "cpu")
  expect_equal(meta$resolved_backend, expected_backend)
})
...
test_that("graph_cluster owns target cluster count", {
  ...
  expect_equal(cl$parameters$n_clusters_requested, 3L)
...
test_that("graph_cluster passes method metric and tuning to internal KNN", {
  ...
  expected_backend <- if (faiss_available()) "faiss_flat_correlation" else "cpu"
  expect_equal(cl$parameters$graph_backend, expected_backend)
  expect_equal(cl$parameters$graph_requested_backend, "cpu")
  expect_equal(cl$parameters$graph_resolved_backend, expected_backend)
})
...
test_that("graph_cluster targets communities on CUDA Louvain and Leiden when libcugraph is available", {
  ...
  for (method in c("louvain", "leiden")) {
    ...
    expect_equal(cl$parameters$n_clusters_requested, 3L, info = method)
